...
<body class="font-sans text-white antialiased">
    <div
        class="min-h-screen w-full bg-brand-dark relative overflow-hidden flex flex-col justify-center items-center py-6 sm:py-0">
        <div id="particles-js" class="absolute inset-0"></div>

        <div class="relative z-10 flex flex-col items-center">
            {{-- Bloque del logo y título ajustado --}}
            <div class="flex flex-col items-center mb-8"> {{-- Contenedor para el logo y el texto, centrado y con margen
                inferior --}}
                <a href="/" class="flex flex-col items-center"> {{-- Enlace que contiene ambos elementos --}}
                    {{-- DIV con fondo blanco para la imagen del logo --}}
                    <div class="bg-white p-1 rounded-md shadow mb-4"> {{-- Añadido mb-4 para separar el logo del texto
                        "Acceso a Grupo Xamanen" --}}
            ...
            <div
                class="w-full sm:max-w-md mt-6 px-6 py-4 bg-white/10 backdrop-blur-sm shadow-xl overflow-hidden rounded-lg border border-white/20">
                @hasSection('content')
                    @yield('content')
                @else
                    {{ $slot ?? '' }}
