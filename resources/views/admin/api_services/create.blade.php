<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <span>{{ __('Nuevo Servicio API') }}</span>
        </div>
    </x-slot>

    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        <x-card>
            <form action="{{ route('admin.api-services.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <x-text-input id="name" name="name" type="text" class="mt-1 block w-full text-black"
                        placeholder="Ej: Mercado Pago" value="{{ old('name') }}" required autofocus />
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>

                <!-- Slug (Opcional) -->
                <div>
                    <x-input-label for="slug" value="Slug / Identificador (Opcional)" />
                    <x-text-input id="slug" name="slug" type="text" class="mt-1 block w-full text-black"
                        placeholder="mercado-pago" value="{{ old('slug') }}" />
                    <p class="text-xs text-gray-500 mt-1">Si lo dejas vacío, se generará automáticamente desde el
                        nombre.</p>
                ...
                <div>
                    ...
                    <x-text-input id="base_url" name="base_url" type="url" class="mt-1 block w-full text-black"
                        placeholder="https://api.mercadopago.com/v1" value="{{ old('base_url') }}" />
                    <x-input-error :messages="$errors->get('base_url')" class="mt-2" />
                </div>

                <!-- Campos Requeridos (Lista Dinámica) -->
                <div x-data="{ fields: [''] }" class="border-t border-gray-200 pt-4 mt-6">
