...
@section('content')
    <div class="max-w-4xl mx-auto py-12">
        <x-card class="bg-white border border-primary-light shadow-lg">
            <x-slot name="header">
                <h2 class="font-headings text-3xl font-extrabold text-primary-dark tracking-tight mb-2">Configuración del
                    Sistema de Correo</h2>
            </x-slot>
            <div class="p-8">
                @if (session('success'))
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input id="mail_mailer" name="mail_mailer" type="text"
                                class="block w-full rounded-md border border-primary-light p-2 text-black"
                                value="{{ old('mail_mailer', $settings['mail_mailer'] ?? 'smtp') }}" required />
                        </div>
                        <div>
                            <label for="mail_host" class="block mb-2 font-bold text-black">Host SMTP (ej:
                                smtp.gmail.com)</label>
                        ...
                        <div>
                            ...
                            <input id="mail_port" name="mail_port" type="number"
                                class="block w-full rounded-md border border-primary-light p-2 text-black"
                                value="{{ old('mail_port', $settings['mail_port'] ?? '587') }}" required />
                        </div>
                        <div>
                            <label for="mail_encryption" class="block mb-2 font-bold text-black">Cifrado (ej: tls,
                                ssl)</label>
                            <input id="mail_encryption" name="mail_encryption" type="text"
                                class="block w-full rounded-md border border-primary-light p-2 text-black"
                                value="{{ old('mail_encryption', $settings['mail_encryption'] ?? 'tls') }}" />
                        </div>
                        ...
                        <div class="md:col-span-2">
                            <label for="mail_password" class="block mb-2 font-bold text-black">Contraseña / App
                                Password</label>
                            <input id="mail_password" name="mail_password" type="password"
                                class="block w-full rounded-md border border-primary-light p-2 text-black" />
                            <p class="text-xs text-gray-500 mt-1">La contraseña no se muestra por seguridad. Déjalo en
                                blanco para mantener la contraseña actual.</p>
                        </div>
                        <div>
